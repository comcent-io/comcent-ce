<script lang="ts">
  ...
  function addNewUsernameOrNumberField() {
    editData.data.to = [...editData.data.to, ''];
  }

  function removeUsernameOrNumber(index: number) {
    editData.data.to = editData.data.to.filter((_, i) => i !== index);
  }

  async function handleInputKeydown(event: any) {
    if (event.key === 'Enter' && currentInput.trim() !== '') {
      event.preventDefault();
      if (editData.data.to.some((item) => item === currentInput)) {
        inputError = 'Already exists';
        return;
      }

      if (isValidPhoneNumber(currentInput)) {
        editData.data.to = [...editData.data.to, currentInput];
        currentInput = '';
      } else {
        const usernameExists = searchResults.some((item) => item.username === currentInput);
        if (!usernameExists) {
  ...
  async function fetchSuggestions(event: any) {
    ...
    if (!isValidPhoneNumber(userInput)) {
      if (userInput.length < 3) {
        ...
        searchProgress = true;
        let tempSearchResults = await searchUser(userInput);
        searchResults = tempSearchResults.filter(
          (item: any) => !editData.data.to.includes(item.username),
        );
  ...
  function selectUser(member: { username: string }) {
    if (!editData.data.to.includes(member.username)) {
      editData.data.to = [...editData.data.to, member.username];
    }
    searchResults = [];
    currentInput = '';
  }
    ...
    <div class="relative w-full max-w-2xl max-h-full">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="p-6 space-y-6">
          ...
          <div class="flex flex-wrap items-center gap-2 p-2 border border-gray-300 rounded">
            {#if editData.data.to.length === 0}
              <span class="bg-gray-600 text-gray-300 text-xs font-semibold px-2 py-0.5 rounded">
                user1
                <svg
            ...
            {#each editData.data.to as usernameOrNumber, index}
              <div
                class="flex items-center bg-blue-100 text-blue-800 text-xs font-semibold px-3 py-1 rounded dark:bg-blue-200 dark:text-blue-800"
              >
                <span>{usernameOrNumber}</span>
                <button
                  class="ml-2 shrink-0 h-full text-blue-500 hover:text-blue-600 dark:text-blue-600 dark:hover:text-blue-700"
                  type="button"
                  on:click={() => removeUsernameOrNumber(index)}
                >
                  <svg
                    class="w-4 h-4"
                    fill="none"
          ...
          <input
            ...
            placeholder="Type a username/number and press Enter to add it to the list."
            required
