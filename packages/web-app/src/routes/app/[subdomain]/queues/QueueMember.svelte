<script lang="ts">
  ...
  interface Props {
    queueMembers?: QueueMemberItem[];
    subdomain?: string;
    queueId?: string;
  }

  let { queueMembers = $bindable([]), subdomain = '', queueId = '' }: Props = $props();

  let searchValue = $state('');
  let searchResults: QueueMemberItem[] = $state([]);
  let searchProgress = $state(false);
  let searchMessage = $state(
    'Type at least 2 characters to search by member name or SIP username.',
  );
...
<section class="space-y-5">
  ...
  >
    ...
    <div class="mt-5 space-y-3">
      ...
      {#if searchResults.length > 0}
        <div
          class="overflow-hidden rounded-xl border border-slate-200 bg-slate-50 shadow-sm dark:border-slate-700 dark:bg-slate-950/60"
        >
            ...
            {#each searchResults as member}
              <li>
                ...
                >
                  ...
                  <span
                    class="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700 dark:bg-blue-950 dark:text-blue-200"
                  >
                    Add to queue
                  </span>
                </button>
  ...
  >
    ...
    {#if queueMembers.length === 0}
      <div
        class="mt-4 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-5 text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-400"
      >
